...
describe('UsagePage credential detail statistics', () => {
  ...
  it.each([1, 2] as const)('keeps observing type %s after reset moves it off the list page and retains data on failure', async (authType) => {
    ...
    const reset = { ...identity(authType, 0), stats_reset_at: '2026-09-10T10:00:00Z' };
    ...
    api.fetchUsageIdentity.mockResolvedValue(identity(authType, 5));
    await tick();
    ...
    await tick();
    expect(metric()).toBe('5');
    api.fetchUsageIdentity.mockResolvedValue(identity(authType, 7));
